...
def aggregate_frames(frame_results: list[dict]) -> dict:
    ...
    avg_faces = (
        statistics.fmean(f.get("faces_detected", 0) for f in frame_results) if n else 0
    )
    primary_face_sizes = [f["face_size_pct"] for f in with_faces if f.get("face_size_pct", 0) > 0]

    thirds_scores = [f["thirds_score"] for f in frame_results
    ...
    brightnesses = [f["mean_brightness"] for f in frame_results
                    if f.get("mean_brightness") is not None]
    laps = [f["laplacian_variance"] for f in frame_results
            if f.get("laplacian_variance") is not None]
    seps = [f["subject_separation_ratio"] for f in frame_results
            if f.get("subject_separation_ratio") is not None]

    head_cutoff_count = sum(1 for f in frame_results if f.get("head_cutoff"))
    no_face_count = sum(1 for f in frame_results if f.get("faces_detected", 0) == 0
                        and f.get("framing") != "no-person")
    very_soft_count = sum(1 for f in frame_results if f.get("focus_rating") == "very-soft")
    sev_under_count = sum(1 for f in frame_results
                          if f.get("exposure_rating") == "severely-underexposed")
    sev_over_count = sum(1 for f in frame_results
                         if f.get("exposure_rating") == "severely-overexposed")
    ...
    intensities = [e["wedding_emotion_score"] for e in emotion_frames
                   if e.get("wedding_emotion_score") is not None]
    dominant_emotions = [e.get("dominant_emotion", "none") for e in emotion_frames
                         if e.get("faces_analyzed", 0) > 0]
    ...
    return {
        ...
        "face_presence": {
            ...
            "primary_face_avg_size_pct": (
                round(statistics.fmean(primary_face_sizes), 2)
                if primary_face_sizes else 0.0
            ),
        ...
        "composition": {
            ...
            "preferred_face_center_y_range": (
                ...
                if face_center_ys else [0.0, 1.0]
            ),
            "avg_headroom_pct": (
                round(statistics.fmean(headrooms), 2) if headrooms else 0.0
            ),
            "headroom_range": (
                [round(_percentile(headrooms, 10), 1),
                 round(_percentile(headrooms, 90), 1)]
                if headrooms else [0.0, 100.0]
            ),
        },

        "exposure": {
            "avg_brightness": (
                round(statistics.fmean(brightnesses), 3) if brightnesses else 0.0
            ),
            "brightness_range": (
                [round(_percentile(brightnesses, 10), 3),
                 round(_percentile(brightnesses, 90), 3)]
                if brightnesses else [0.0, 1.0]
            ),
        },
        ...
        "subject_separation": {
            "avg_center_to_edge_ratio": (
                round(statistics.fmean(seps), 2) if seps else 1.0
            ),
...
def learn_from_thumbnails(thumbs_root: Path,
                          films: list[str] | None = None,
                          progress_cb=None,
                          analyze_frame_fn=None,
                          analyze_emotion_fn=None) -> dict:
    ...
    if not film_dirs:
        raise FileNotFoundError(
            f"no analyzed films found under {thumbs_root}; "
            "run `film-style analyze` first."
        )
