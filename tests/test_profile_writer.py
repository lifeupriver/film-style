...
def _mk(filename, dur, clip_specs, *, color=None, music=None, audio=None,
        chapters=None, shot_sizes=None):
    clips = []
    for i, (a, b, t_out) in enumerate(clip_specs):
        c = Clip(index=i, start_sec=a, end_sec=b, duration_sec=b - a,
    ...
    return FilmAnalysis(
        ...
        film=FilmMeta(filename=filename, path=f"/tmp/{filename}", duration_sec=dur,
        ...
        pacing=Pacing(avg_clip_duration_sec=avg, median_clip_duration_sec=avg,
                      std_dev_sec=0.5, min_clip_sec=min(durations),
        ...
        structure=Structure(
            ...
            closing={"last_cut_at_sec": 0.0, "last_5_clips_avg_duration_sec": avg,
...
def test_profile_with_color_emits_grade_rule():
    color = {"mean_luminance": 110.0, "mean_contrast": 50.0,
             "mean_warm_cool": 0.35, "mean_saturation": 100.0,
...
def test_profile_with_scene_breakdown():
    chapters = [
        ChapterRecord(index=0, start_clip=0, end_clip=0, start_sec=0, end_sec=10,
                      duration_sec=10, clip_count=1, avg_clip_sec=10.0, label="ceremony"),
        ChapterRecord(index=1, start_clip=1, end_clip=1, start_sec=10, end_sec=20,
                      duration_sec=10, clip_count=1, avg_clip_sec=2.0, label="dancing"),
    ]
    f = _mk("a.mp4", 20,
