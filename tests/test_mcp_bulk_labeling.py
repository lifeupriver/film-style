...
def _write_film(mcp_server, stem, *, n_chapters=4, n_clips=10):
    ...
    payload = {
        ...
        "pacing": {
            "avg_clip_duration_sec": 6.0,
            "median_clip_duration_sec": 6.0,
            "std_dev_sec": 0.0,
            "min_clip_sec": 6.0,
            "max_clip_sec": 6.0,
...
def test_get_undescribed_clip_thumbnails_returns_schema_hint(fixture_home):
    ...
    assert out["remaining_after_batch"] == 2
    assert "scene_labels" in out["schema_hint"]
    assert "ceremony" in out["schema_hint"]["scene_labels"]
...
def test_set_clip_descriptions_bulk_writes_structured(fixture_home):
    ...
    res = mcp_server.tool_set_clip_descriptions_bulk(
        "alpha",
        {
            "0": {
...
def test_set_clip_descriptions_bulk_clears_with_none(fixture_home):
    ...
    mcp_server.tool_set_clip_descriptions_bulk(
        "alpha",
        {
